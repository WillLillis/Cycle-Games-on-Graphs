...
// Everything is currently commented out because 
...
// *(avail_threads[curr_threads].thread) = &std::thread(MAC_threaded_dispatch, curr_neighbor, num_nodes, adj_matrix, &avail_threads[curr_thread]);
// // *(avail_threads[i].thread)->joinable
//if (avail_threads[i].thread.joinable())

//Need to completely redo this using std::vector's
...
//	std::thread thread;
//	//pthread_t my_thread;
//	uint_fast16_t* edge_use_matrix;
//	uint_fast16_t* node_use_list;
//	std::atomic_bool* kill_flag;
//	GAME_STATE return_val;
//	bool avail_for_use;
//	//const uint_fast16_t curr_node, const uint_fast16_t num_nodes, const uint_fast16_t* adj_matrix, 
//	//THREAD_GAME_INFO* thread_materials)
//	//THREAD_GAME_INFO(uint_fast16_t num_nodes_in, uint_fast16_t* adj, GAME_STATE(*func)(const uint_fast16_t, const uint_fast16_t, const uint_fast16_t*))
//	//{
//	//	// replace with std::vector's
//	//	edge_use_matrix = (uint_fast16_t*)calloc(num_nodes_in * num_nodes_in * sizeof(uint_fast16_t), sizeof(uint_fast16_t));
//	//	node_use_list = (uint_fast16_t*)calloc(num_nodes_in, sizeof(uint_fast16_t));
//	//	kill_flag = (std::atomic_bool*)malloc(sizeof(std::atomic_bool));
//
//	//	return_val = ERROR_STATE;
//	//	avail_for_use = true;
//	//	std::thread my_thread(func, 0, 10, adj);
//	//};
//}THREAD_GAME_INFO;
//
