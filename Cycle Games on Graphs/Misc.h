/*
 *
 * Just going to throw all the random helper functions in here that don't
 * logically fit within the other files
 *
 */
#pragma once
#include <cmath>
...
// It seems reasonable to only worry about the Miscrosoft, GCC, and CLANG
// compilers, but if we need to add more later on, then we can do so Since we're
// using Microsoft's __FUNCSIG__ by default, we really only need to check for
// CLANG AND GCC when WIN32 isn't defined WIN32 or _WIN32?-> WIN32 for the
// compiler, _WIN32 for the OS?
// Add some #pragma message()'s here to inform person building what macro is
// being used?
...
#define __FUNCSIG__                                                            \
    __func__ // sources online say __FUNCTION__ should work pretty much
             // everywhere, but __func__ is more standardized
#endif       // __clang__ || __GNUC__
#endif       // !WIN32

// going to define the below function-like macro expression to ease the use of
// the display_error function someone editing the code can skip passing in
// __FILE__, _LINE__, __FUNCSIG__ as the first args to display_error...
// ...and instead simply type DISPLAY_ERR(user_clear, err_msg, ...)
#define DISPLAY_ERR(user_clear, err_msg, ...)                                  \
    display_error(__FILE__, __LINE__, __FUNCSIG__, user_clear,                 \
                  err_msg __VA_OPT__(, ) __VA_ARGS__)
 ...
 * __LINE__ macro
 * - func_sig : the signature of the function in which the error occurred,
 *grabbed using the __FUNCSIG__ macro (or by whatever it's being used as an
 *alias for, depending on the compiler)
 * - user_clear : indicates whether the user will have to provide an input in
 *order to clear/ continue past the error
...
void display_error(const char *__restrict file_name, const int line_num,
                   const char *__restrict func_sig, const bool user_clear,
                   const char *__restrict err_msg, ...) {
    printf("ERROR: ");
 ...
 * - none
 ****************************************************************************/
// combine with erase_lines function, allow some special argument to specify
// erasing all lines
inline void clear_screen() {
    ...
    if ((err = system("clear")) != 0) {
        DISPLAY_ERR(false,
                    "Failed to clear screen. Error code returned from "
                    "\"system\" call: %d.",
                    err);
    }
#else
    DISPLAY_ERR(false, "Failed to clear the screen. Unable to identify "
...
void erase_lines(const uint_fast16_t num_lines) {
    if (num_lines > 0) [[likely]] {
        ...
        for (uint_fast16_t line = 1; line < num_lines;
             line++) {         // line = 1 because we included the first line
 ...
 ****************************************************************************/
inline size_t index_translation(const uint_fast16_t num_cols,
                                const uint_fast16_t row,
                                const uint_fast16_t col) {
...
size_t get_file_length(std::fstream *__restrict file) {
    ...
    (*file).clear(); // clear the EOF bit set by our going to the end of the
                     // file, any other errors
 ...
 ****************************************************************************/
template <typename T> inline size_t num_digits(const T input) {
    // compile time check because function will only work with ints
    static_assert(!std::is_floating_point_v<T>,
                  "Function template num_digits() only accepts integer types!");
    if (input == 0) {
        return 1;
    } else if (input > 0) {
        return (size_t)std::log10(std::abs((const int64_t)input)) + 1;
    } else {
        return (size_t)std::log10(std::abs((const int64_t)input)) +
               2; // negatives need an additional character for the '-' sign
    }
}
